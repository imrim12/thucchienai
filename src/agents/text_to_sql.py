...
from src.llm.google import get_gemini_embeddings, get_gemini_llm
...
class TextToSQLService:
        ...
        try:
            ...
            if self.target_db is None:
                ...
                if isinstance(response.content, list):
                    raw_sql = (
                        str(response.content[0]).strip() if response.content else ""
                    )
                else:
    ...
    def process_question(
        self, natural_question: str, readonly: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            similar_result = self.cache.find_similar_question(
                question_vector, self.settings.SIMILARITY_THRESHOLD
            )
            ...
            if similar_result:
                ...
                print(
                    f"Found cached SQL for similar question: {cached_question[:50]}..."
                )

                # Validate cached SQL against readonly constraint
                validated_sql = self.validator.validate_and_clean_sql(
                    cached_sql, readonly
                )

                if validated_sql:
                    print("Cached SQL passed validation")
                    return {
                    ...
                    print(
                        "Cached SQL failed readonly validation, generating new query..."
                    )
    ...
    def execute_sql(self, sql_query: str) -> Dict[str, Any]:
        ...
        """
        if self.target_db is None:
            return {"success": False, "error": "No target database configured"}

        try:
            result = self.target_db.run(sql_query)
            return {"success": True, "result": result, "query": sql_query}
        except Exception as e:
    ...
    def validate_sql_with_llm(
        self, sql_query: str, readonly: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            local_validation = self.validator.validate_and_clean_sql(
                sql_query, readonly
            )
            ...
            if isinstance(response.content, list):
                corrected_sql = (
                    str(response.content[0]).strip() if response.content else sql_query
                )
            else:
                corrected_sql = str(response.content).strip()

            # Validate the LLM's correction
            final_validation = self.validator.validate_and_clean_sql(
                corrected_sql, readonly
            )
            ...
            return {
                ...
                if final_validation
                else "SQL could not be corrected.",
            }
