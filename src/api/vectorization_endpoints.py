...
from src.database.models import DatabaseConnection, TableConfig
...
def discover_database_schema(db_id: int):
    ...
    try:
        ...
        return jsonify(
            {"message": "Database discovery completed", "summary": summary}
        ), 200
...
@vectorization_bp.route(
    "/databases/<int:db_id>/tables/<table_name>/configure", methods=["POST"]
)
...
def start_vectorization(table_config_id: int):
    ...
    try:
        ...
        job = vectorization_service.start_vectorization_job(
            table_config_id=table_config_id, user_id=user_id
        )
...
def process_vectorization_job(job_id: int):
    ...
    try:
        ...
        if success:
            ...
            return jsonify(
                {"message": "Vectorization job failed", "job_id": job_id}
            ), 500
...
def search_vectorized_content():
    ...
    try:
        ...
        return jsonify(
            {"query": data["query"], "results": results, "total_results": len(results)}
        ), 200
...
def list_database_connections():
    ...
    try:
        with get_db_session_context() as session:
            ...
            return jsonify({"databases": result, "total": len(result)}), 200
...
def list_table_configurations():
    ...
    try:
        with get_db_session_context() as session:
            tables = (
                session.query(TableConfig).filter(TableConfig.is_enabled == True).all()
            )
                ...
                result.append(
                    {
                        ...
                        "last_vectorized": table.last_vectorized.isoformat()
                        if table.last_vectorized
                        else None,
                    }
                )
