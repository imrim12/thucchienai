...
class VectorizationService:
    ...
    def start_vectorization_job(
        self, table_config_id: int, user_id: Optional[str] = None
    ) -> VectorizationJob:
        """
        ...
        with get_db_session_context() as session:
            ...
            existing_job = (
                ...
                .filter(
                    VectorizationJob.status.in_(
                        [VectorizationStatus.PENDING, VectorizationStatus.IN_PROGRESS]
                    )
                )
                .first()
            )
            ...
            if existing_job:
                raise ValueError(
                    "A vectorization job is already running for this table"
                )
            ...
            logger.info(
                f"Created vectorization job {job.id} for table {table_config.table_name}"
            )
            return job
    ...
    def process_vectorization_job(self, job_id: int) -> bool:
        ...
        try:
            with get_db_session_context() as session:
                ...
                logger.info(
                    f"Starting vectorization job {job_id} for table {table_config.table_name}"
                )
                ...
                session.flush()
                logger.info(
                    f"Completed vectorization job {job_id}: {total_successful}/{total_processed} records"
                )
                return True
    ...
    def _process_table(
        ...
        try:
            ...
            if not vectorizable_columns:
                logger.warning(
                    f"No vectorizable columns found for table {table_config.table_name}"
                )
                return 0, 0, 0
            ...
            if collection_name:
                ...
                for batch in self._get_table_batches(
                    engine, table_config, vectorizable_columns
                ):
                    ...
                    if job.total_rows > 0:
                        job.progress_percentage = (
                            total_processed / job.total_rows
                        ) * 100
                    session.flush()

                    logger.info(
                        f"Job {job.id}: Processed {total_processed} records ({successful} successful, {failed} failed)"
                    )
            else:
                logger.warning(
                    f"No ChromaDB collection name configured for job {job.id}"
                )
    ...
    def _get_table_batches(
        self, engine: Engine, table_config: TableConfig, columns: List[ColumnConfig]
        ...
        with engine.connect() as conn:
            while True:
                ...
                batch = []
                for row in rows:
                    record = dict(zip(result.keys(), row, strict=False))
                    batch.append(record)
                ...
                if column.should_vectorize and column.embedding_weight > 0:
                    ...
                    if value is not None:
                        ...
                        weighted_parts.append(
                            f"{column.column_name}: {weighted_content}"
                        )
    ...
    def _generate_metadata(
        self, record: Dict[str, Any], table_config: TableConfig
        ...
        try:
            ...
            if self.chroma_cache.client:
                self.chroma_cache.client.create_collection(
                    ...
                    metadata={"description": "Vectorized content from database table"},
                )
        ...
        try:
            ...
            if collection_name:
                ...
                if not self.chroma_cache.client:
                    return []
                collections = [
                    col.name for col in self.chroma_cache.client.list_collections()
                ]
