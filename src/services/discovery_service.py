...
from src.database.models import DatabaseConnection, TableConfig, ColumnConfig, DatabaseType
...
class DatabaseDiscoveryService:
    ...
    def __init__(self):
        self.supported_text_types = {
            ...
            'longtext', 'mediumtext', 'tinytext', 'string', 'json',
            'jsonb', 'xml', 'clob', 'blob'
        }
    ...
    def discover_database_schema(self, connection_string: str, database_type: str) -> Dict[str, Any]:
        """
    ...
    def _analyze_table(self, inspector, table_name: str, engine: Engine) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            table_info['vectorization_potential'] = self._calculate_vectorization_potential(table_info)
    ...
    def _analyze_column(self, column: Dict[str, Any]) -> Dict[str, Any]:
        ...
        is_text = any(text_type in column_type for text_type in self.supported_text_types)
        is_numeric = any(num_type in column_type for num_type in self.supported_numeric_types)
        ...
        if 'text' in column_type or 'longtext' in column_type:
            potential_score = 0.9
        elif 'varchar' in column_type and ('description' in column['name'].lower() or 
                                           'comment' in column['name'].lower() or
                                           'content' in column['name'].lower()):
            potential_score = 0.8
    ...
    def _estimate_table_size(self, engine: Engine, table_name: str) -> int:
        ...
        try:
            with engine.connect() as conn:
                ...
                result = conn.execute(text(f"SELECT COUNT(*) FROM {table_name} LIMIT 100000"))
    ...
    def _calculate_vectorization_potential(self, table_info: Dict[str, Any]) -> float:
        ...
        vectorizable_ratio = table_info['vectorizable_columns'] / table_info['total_columns']
        ...
        potential = (text_ratio * 0.4 + vectorizable_ratio * 0.6 + content_bonus) * size_factor
    ...
    def auto_configure_table(
        ...
        with get_db_session_context() as session:
            ...
            table_config = TableConfig(
                ...
                primary_key_column=table_info['primary_key_columns'][0] if table_info['primary_key_columns'] else None,
                total_records=table_info['estimated_rows'] if table_info['estimated_rows'] > 0 else 0
            )
