...
class ChromaCache:
    ...
    def __init__(
        ...
        collection_name: str = "sql_queries",
    ):
        """
        ...
        try:
            self.collection: Optional[Collection] = self.client.get_collection(
            ...
            self.collection = self.client.create_collection(
                name=collection_name,
                metadata={
                    "description": "Text-to-SQL query cache with vector similarity search"
                },
            )
    ...
    def add_to_cache(
        self, natural_question: str, sql_query: str, question_vector: np.ndarray
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            self.collection.add(
                ...
                metadatas=[
                    {"sql_query": sql_query, "timestamp": str(np.datetime64("now"))}
                ],
                ids=[doc_id],
            )
    ...
    def find_similar_question(
        self, question_vector: np.ndarray, threshold: float = 0.8
        ...
        try:
            ...
            if (
                ...
                and cast(Any, results["documents"])
                and cast(Any, results["distances"])
                and len(cast(Any, results["documents"])[0]) > 0
            ):
                ...
                if similarity_score >= threshold:
                    ...
                    print(
                        f"Found similar question with similarity: {similarity_score:.3f}"
                    )
                    return original_question, sql_query, float(similarity_score)
                else:
                    print(
                        f"Best match similarity {similarity_score:.3f} below threshold {threshold}"
                    )
