...
# Import enums
from .enums import DatabaseType, VectorizationStatus, VectorizationStrategy
...
def get_enabled_table_configs(
    session: Session, database_connection_id: Optional[int] = None
) -> List[TableConfig]:
    ...
    if database_connection_id:
        query = query.filter(
            TableConfig.database_connection_id == database_connection_id
        )
    return query.all()


def get_vectorizable_columns(
    session: Session, table_config_id: int
) -> List[ColumnConfig]:
