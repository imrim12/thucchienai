...
class TableConfig(Base):
    ...
    vectorization_strategy: Mapped[str] = mapped_column(
        String(50), default="full_table", nullable=False
    )  # VectorizationStrategy enum
    ...
    )  # Auto-reprocess on data changes
    update_frequency: Mapped[Optional[str]] = mapped_column(
        String(20)
    )  # daily, weekly, monthly

    # Custom processing settings (JSON)
    processing_config: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON
    )  # Custom options, filters, etc.
    ...
    database_connection = relationship(
        "DatabaseConnection", back_populates="table_configs"
    )
    column_configs = relationship(
        "ColumnConfig", back_populates="table_config", cascade="all, delete-orphan"
    )
    vectorization_jobs = relationship(
        "VectorizationJob", back_populates="table_config", cascade="all, delete-orphan"
    )
