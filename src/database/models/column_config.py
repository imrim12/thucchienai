...
class ColumnConfig(Base):
    ...
    column_type: Mapped[Optional[str]] = mapped_column(
        String(50)
    )  # e.g., 'varchar', 'text', 'int', etc.

    # Vectorization settings
    should_vectorize: Mapped[bool] = mapped_column(
        Boolean, default=False, nullable=False
    )
    embedding_weight: Mapped[float] = mapped_column(
        Float, default=1.0
    )  # Weight for this column in the final embedding

    # Processing options
    preprocessing_config: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON
